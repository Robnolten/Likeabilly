...
// Error logging
ini_set('display_errors', 1); // 0 = Geen fouten tonen aan gebruiker
...
// --- Database Verbindingsgegevens voor LOKALE MAMP ---
...
try {
    ...
    $status_default = 'pending'; // Verander 'received' naar 'pending' 
    ...
    // --- Stap 1: Mollie Betaling Initiëren ---
    error_log("Initiating Mollie payment for order ID: " . $orderId);
    $mollieApiKey = 'test_QEq8MPduhc5wD6K3EWwrwTHCx7RWjs'; // <<< VUL HIER JE ECHTE MOLLIE *TEST* API KEY IN!

    // Controleer of API key is ingevuld (basale check)
    if (strpos($mollieApiKey, 'test_') !== 0) {
        throw new \Exception("Mollie Test API Key not set correctly.");
    }

    try {
        $mollie = new \Mollie\Api\MollieApiClient();
        $mollie->setApiKey($mollieApiKey);
        error_log("Mollie client initialized.");

// Maak de betaling aan bij Mollie
$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";
// Gebruik localhost voor redirect/webhook tijdens MAMP test
$domain = "localhost"; // Pas aan voor live! MAMP draait op poort 80.
$redirectUrl = $protocol . '://localhost:5173/bedankt?orderId=' . $orderId; // Terug naar Vite dev server
$webhookUrl = 'https://dbd1-85-146-191-117.ngrok-free.app'; // Naar MAMP

error_log("Creating Mollie payment with Amount: {$price}, Redirect: {$redirectUrl}, Webhook: {$webhookUrl}");

$payment = $mollie->payments->create([ // <<<< ----- DEZE REGEL ZOEK JE
    "amount" => [
        "currency" => "EUR",
        "value" => number_format($price, 2, '.', '') // Formaat: "123.45"
    ],
    "description" => "Likeabilly Bestelling #" . $orderId,
    "redirectUrl" => $redirectUrl,
    "webhookUrl"  => $webhookUrl,
    "metadata" => [
        "order_id" => $orderId, // Koppel order ID aan Mollie betaling
    ],
]);

    // De bestaande code gaat hier verder:
    error_log("Mollie payment created with ID: " . $payment->id);
    $checkoutUrl = $payment->getCheckoutUrl();
    $paymentId = $payment->id;

    // --- Stap 3: Mollie Payment ID Opslaan bij de Order (NIEUW TOEGEVOEGD) ---
    try {
        error_log("Attempting to update order {$orderId} with Mollie payment ID: {$paymentId}");
        // Gebruik de bestaande PDO connectie ($pdo) die eerder is gemaakt
        $sql_update = "UPDATE orders SET mollie_payment_id = :payment_id WHERE id = :order_id";
        $stmt_update = $pdo->prepare($sql_update);
        $stmt_update->execute([':payment_id' => $paymentId, ':order_id' => $orderId]);
        $rowCountUpdate = $stmt_update->rowCount();
        if ($rowCountUpdate > 0) {
             error_log("Order {$orderId} updated successfully with Mollie payment ID.");
        } else {
             error_log("WARN: Order {$orderId} update with Mollie payment ID {$paymentId} affected 0 rows. Was ID correct?");
        }
    } catch (\PDOException $e_update) {
        // Log de fout, maar stop het script niet per se. De webhook is belangrijker voor de status.
        error_log("DATABASE UPDATE ERROR (Mollie ID): Failed to update order {$orderId} with Mollie ID {$paymentId}. Error: " . $e_update->getMessage());
        // Optioneel: Je zou hier kunnen besluiten de $checkoutUrl op null te zetten of een interne melding te sturen.
        // Voor nu loggen we alleen de fout en gaan door.
    }
    // --- Einde NIEUWE CODE ---

    } catch (\Mollie\Api\Exceptions\ApiException $e_mollie_api) {
        error_log("Mollie API Error: " . $e_mollie_api->getMessage() . " - Field: " . $e_mollie_api->getField()); // <-- getPlainResponse() verwijderd!
        throw new \Exception("Failed to initialize Mollie payment: " . $e_mollie_api->getMessage()); // Gooi opnieuw om door hoofd catch te worden gevangen
    } catch (\Throwable $e_mollie_general) {
        error_log("General Mollie Setup Error: " . $e_mollie_general->getMessage());
        throw $e_mollie_general; // Gooi opnieuw
    }
    // --- Einde Mollie Initiatie (voor nu) ---
    ...
    */
    ...
    // Succesvolle afronding - Stuur Mollie info terug
    error_log("Preparing JSON response with Mollie details");
    http_response_code(200);
    echo json_encode([
        'status' => 'success',
        'message' => "Pending order #{$orderId} saved, payment initiated.",
        'orderId' => $orderId,
        'paymentId' => $paymentId,    // Stuur Mollie Payment ID mee
        'checkoutUrl' => $checkoutUrl   // Stuur Mollie Checkout URL mee
    ]);
    error_log("JSON response sent with Mollie details for Order ID: {$orderId}, Payment ID: {$paymentId}");
    exit;
