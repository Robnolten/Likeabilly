...
const handleBestellen = async () => {
  ...
  // --- Gebruik state voor klantgegevens ---
  const customerData = {
    name: customerName,
    email: customerEmail,
    ...
    zip: customerZip,
    city: customerCity,
    phone: customerPhone, // Optioneel, wordt meegestuurd indien ingevuld
    notes: customerNotes, // Optioneel, wordt meegestuurd indien ingevuld
    createAccount: createAccount, // true of false
    // Stuur wachtwoord alleen mee als account creatie is aangevinkt (backend moet hier veilig mee omgaan!)
    password: createAccount ? password : undefined,
  };

  // --- Validatie klantgegevens ---
  if (!customerName || !customerEmail || !customerAddress || !customerZip || !customerCity) {
      ...
      // Optioneel: Scroll naar het eerste lege veld?
  ...
  // --- Verzamel configuratie data ---
  const calculatedSpaces = calculateSpacesBetweenShelves();
  // Herbereken totalShelfCount hier voor de zekerheid
  ...
  const configurationData = {
      ...
      inputDepth: depth,
      material: material,
      shelvesPerCompartment: shelves, // Aantal planken per vak zoals ingesteld
      isCustomLayout: customLayout,
      finalShelfPositions: shelfPositions, // Array met Y-posities (midden) van planken
      calculatedPrice: prijs, // De berekende prijs uit useMemo
      calculatedNumberOfDividers: numberOfDividers, // Aantal tussenschotten
      calculatedTotalCompartments: currentTotalCompartments, // Totaal aantal vakken
      calculatedTotalShelfCount: currentTotalShelfCount, // Totaal aantal planken
      shelfSpaces: calculatedSpaces, // Array met objecten {from, to, space}
      // Stuur eventueel andere relevante berekende waarden mee
      shelfThickness: shelfThickness,
      kickboardHeight: kickboardHeightConst, // Gebruik de constante
      // Berekende binnenmaten (kunnen handig zijn voor backend)
      calculatedInnerWidth: width - 2 * shelfThickness,
      calculatedInnerDepth: depth - 0.36, // Gebruik de constante aftrekDiepte hier?
      calculatedAvailableInsideHeight: availableInsideHeight
  };

  // Stel de volledige payload samen
  const orderPayload = {
    configuration: configurationData,
    customer: customerData,
  };

  // Het pad naar je PHP script op de server
  const apiUrl = '/api/orders.php';

  // --- Fetch call naar backend ---
  try {
    console.log('Sending order data:', orderPayload); // Log de data die verstuurd wordt

    const response = await fetch(apiUrl, {
       method: 'POST',
       headers: {
         'Content-Type': 'application/json',
         // Voeg eventuele andere headers toe indien nodig
       },
       body: JSON.stringify(orderPayload), // Zet het JS object om naar JSON string
    });

    // Lees altijd de response body, zelfs bij fouten, indien mogelijk
    let responseData;
    try {
        responseData = await response.json(); // Probeer JSON response te lezen
    } catch (jsonError) {
        console.error("Failed to parse JSON response:", jsonError);
        // Als het geen JSON is, probeer als tekst (voor PHP fouten bv)
        try {
    ...
    // Controleer of de server een succesvolle HTTP status gaf (2xx)
    if (!response.ok) {
    ...
    alert(`Bestelling succesvol ontvangen door server! Bericht: ${responseData.message}`);

    // TODO: Hierna komt de logica voor na een succesvolle order
    // Bijv. doorsturen naar betaalpagina (URL uit responseData?)
    // Of naar een bedankpagina.
    // if (responseData.orderId) { window.location.href = `/betalen?order=${responseData.orderId}`; }
    // else { window.location.href = '/bedankt'; }

  } catch (error) {
    // Vang netwerkfouten of andere onverwachte JS fouten op
      ...
      <div className="flex flex-col md:flex-row h-screen overflow-hidden">
        ...
        {isMobile ? (
          ...
          <>
            ...
            {/* Controls */}
            <div className={`w-full ${expandedControls ? 'h-2/3' : 'h-1/3'} p-4 bg-gray-100 overflow-auto overscroll-contain relative transition-all duration-300`}>
          ...
          <>
            <div className="w-full md:w-1/3 p-6 bg-gray-100 overflow-auto overscroll-contain">
