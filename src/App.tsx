    ...
    export default function App() {
      ...
      const MOLLIE_PROFILE_ID = 'pfl_9GTRhNJzSh'; // <<< VUL HIER JE ECHTE PROFILE ID IN!
      ...
      const navigateToStep = (newStep: number) => {
        if (newStep >= 1 && newStep <= 5) {
          setStep(newStep);
          // HIER TOEGEVOEGD: Reset checkout view als we naar stap 1-4 gaan
          if (newStep < 5) {
      ...
      const handleProceedToPayment = async () => {
        console.log('*** handleProceedToPayment EXECUTED - Direct Call Test ***'); // Eerste regel
        // Rest van de originele functie (of houd leeg voor test)
      };
...
// BEGIN HANDLEBESTELLEN

const handleBestellen = async () => {
  console.log('Bestellen gestart vanuit stap 5...');

  // Klantgegevens valideren
  console.log(">> Stap A: Klantgegevens controleren:", customerName, customerEmail, customerAddress);
  if (!customerName || !customerEmail || !customerAddress || !customerZip || !customerCity) {
    alert("Vul alle verplichte velden (*) in bij uw gegevens.");
    return;
  }
  ...
  console.log(">> Stap B: Orderpayload wordt gemaakt");
  ...
  console.log(">> Stap C: Orderpayload klaar:", orderPayload);

  const apiUrl = '/api/orders.php';

  console.log(">> Stap D: Versturen order data naar backend op URL:", apiUrl);
  ...
  try {
    ...
    console.log(">> Stap E: Fetch-call uitgevoerd, response ontvangen:", response);
    ...
    try {
      ...
      console.error("Ruwe response:", textResponse);
      responseData = { message: textResponse || "Server gaf onleesbaar antwoord." };
    }

    if (!response.ok) {
      console.error('Error response from server:', responseData);
      alert(`Fout bij het versturen van de bestelling: ${responseData.message || `HTTP status ${response.status}`}`);
      return;
    }
    ...
    if (responseData.checkoutUrl) {
      ...
      alert("Er ging iets mis met het starten van de betaling. Probeer opnieuw.");
    }
    // Voeg hier eventueel verdere logica toe (zoals doorsturen naar Mollie)

  } catch (error) {
    console.error('Network error or issue sending order:', error);
    alert('Er ging iets mis met de verbinding tijdens het versturen. Probeer het opnieuw.');
  }
};

 //EINDE HANDLEBESTELLEN
      ...
      return (
        <div className="flex flex-col md:flex-row h-screen overflow-hidden">
          ...
          {isMobile ? (
                  ...
                  {step === 5 && (
                      ...
                      >
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="pt-4">
                            <button
                              onClick={handleBestellen} // Gekoppeld aan de (nog te vervangen) handleBestellen
                              className="w-full bg-indigo-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-indigo-700 transition-colors"
                            >
                              Bestelling Plaatsen & Betalen {/* Tekst eventueel aanpassen */}
                            </button>
                          </div>
            ...
            <>
              <div className={`w-full ${isCheckoutActive ? 'md:w-2/3' : 'md:w-1/3'} p-6 pt-16 bg-gray-100 overflow-auto overscroll-contain touch-pan-y h-[calc(100vh-0px)] mt-[0px] transition-all duration-500 ease-in-out`}>          
              <h2 className="text-xl font-semibold mb-4">Maak je kast op maat</h2>
                <NewStepProgressBar activeStep={step} onStepClick={navigateToStep} />
                { !isCheckoutActive ? (
                <AnimatePresence mode="wait">
                  {step === 1 && (
                  ...
                  {step === 5 && (
                      ...
                      >
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="pt-4">
                          <button
                            onClick={() => setIsCheckoutActive(true)} // <-- TERUG GEZET!
                            className="w-full bg-indigo-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-indigo-700 transition-colors">
                            Bekijk je bestelling{/* Tekst eventueel aanpassen */}
                          </button>
                        </div> {/* Einde formulier container */}
                        </div>
                      </motion.div>
                ...
                // -------- START NIEUWE CODE VOOR CHECKOUT WEERGAVE --------
                <motion.div
                key="checkout" // Key for animation transition
                initial="initial"
                animate="animate"
                exit="exit"
                variants={pageVariants} // Reuse existing animation variants
                className="bg-white p-6 rounded-xl shadow mb-4" // Ruimere padding (p-6)
                >
                <h3 className="text-xl font-bold mb-4 text-indigo-800">Afrekenen & Overzicht</h3>

                {/* Placeholder voor Order Overzicht */}
                <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
                    <h4 className="font-semibold mb-2 text-gray-700">Je Configuratie:</h4>
                    {/* We gebruiken hier direct de state variabelen */}
                    <ul className="text-sm space-y-1 text-gray-600">
                        <li>Hoogte: {height} cm</li>
                        <li>Breedte: {width} cm</li>
                        <li>Diepte: {depth} cm</li>
                        <li>Materiaal: <span className="capitalize">{material}</span></li>
                        <li>Aantal planken: {totalShelfCount}</li> {/* Gebruik berekende totalShelfCount */}
                        {customLayout && <li>Indeling: Aangepast</li>}
                    </ul>
                    <p className="mt-3 font-bold text-indigo-700">Geschatte Prijs: €{prijs}</p>
                </div>

    {/* --- START Correct Conditioneel Blok --- */}
    {!molliePaymentId ? (
                    // --- FASE 1: Toon ALLEEN knop als er nog geen paymentId is ---
                    <>
                      {/* "Bevestig & Betaal" knop */}
                      <button onClick={handleBestellen}
                        className="w-full bg-green-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-green-700 transition-colors mb-2"
                      >
                        Bevestig & Betaal
                      </button>
                      {/* HIER GEEN <MolliePaymentForm> */}
                    </>
                  ) : (
                    // --- FASE 2: Toon ALLEEN Mollie Components als paymentId WEL bestaat ---
                    <div className="mb-6">
                      <h4 className="font-semibold mb-2 text-gray-700">Voer betaling uit:</h4>
                      {/* Het Mollie component wordt NU HIER gerenderd */}
                      <MolliePaymentForm
                        paymentId={molliePaymentId} // Nu met een geldige ID
                        profileId={MOLLIE_PROFILE_ID}
                        onPaymentComplete={handleMolliePaymentComplete} // Je aangepaste handler
                        onPaymentError={(error) => {
                            console.error("Mollie Component Error reported:", error);
                      ...
                      />
                      {/* HIER GEEN placeholder div meer nodig */}
                    </div>
                    // "Bevestig & Betaal" knop is nu verborgen omdat we in de else-tak zitten.
                  )}
                  {/* --- EINDE Correct Conditioneel Blok --- */}
                {/* --- EINDE Conditioneel Blok --- */}


                {/* "Terug" knop blijft altijd zichtbaar in checkout view */}
                <button
                    onClick={() => {
                        setIsCheckoutActive(false); // Ga terug naar formulier view
                        setMolliePaymentId(null);  // Reset payment ID bij teruggaan!
                    }}
                    className="w-full mt-2 px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-gray-800 transition-colors font-medium"
                >
                    Terug naar gegevens aanpassen
                </button>
                </motion.div>
                // -------- EINDE NIEUWE CODE VOOR CHECKOUT WEERGAVE --------
                )}
                ...
                <button
                  onClick={() => {
                    ...
                    setIsCheckoutActive(false); // HIER TOEGEVOEGD!
                  }}
                  className="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400 text-gray-800 transition-colors"
                >
                  Vorige
              ...
              <div className={`w-full ${isCheckoutActive ? 'md:w-1/3' : 'md:w-2/3'} h-[calc(100vh -0)] relative pt-16 mt-[0px] transition-all duration-500 ease-in-out`}>
